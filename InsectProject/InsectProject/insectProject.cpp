...
SECTION : 004
HOMEWORK: 2 (Sad Death of Insects)
----------
PROBLEMS: I don't know the structure because I didn't take 152 classes.
          Although I took great care in my drawings, I could not achieve any function.
          I was very pleased that you liked my idea, but I could not implement it.
...
double insectX = 329, insectY = -297; // my variables for movement insect(s)
...
void displayInsectPowder(int x, int y)
{
    ...
    circle_wire(x + 55, y + 13, 9);

    // the tip of a drop of insecticide
    glColor3ub(25, 162, 34);
    glBegin(GL_TRIANGLES);
    glVertex2f(x + 30, y + 13); //A - top of triangle
    glVertex2f(x + 55, y + 22); //B top
    glVertex2f(x + 55, y + 4); //C bottom
    glEnd();

    // the frame of the tip of a drop of insecticide
    glColor3ub(18, 125, 25);
    glBegin(GL_LINES);
    glVertex2f(x + 30, y + 13);
...
void display()
{
    ...
    displayInsectPowder(handX, handY);
    displayInsect(insectX, insectY);
    displayScreen();
    glColor3ub(192, 192, 192);
    vprint2(-655, 270, 0.3, "00:%02d", time);
    glColor3ub(192, 192, 192);
    vprint2(-650, 320, 0.25, "Remaining Time");
    glLineWidth(0.61);
...
void sprayTimer(int v)
{
    glutPostRedisplay();
    glutTimerFunc(5, sprayTimer, 0);
...
int randomXforInsect()
{
    int number = rand() % 570;
    return number;
}

void insectTimer(int v)
{
    insectX = randomXforInsect();
    ...
    glutTimerFunc(20, insectTimer, 0);
    glutPostRedisplay();
}
